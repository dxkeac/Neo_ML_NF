...
def get_specific_length_peptide(sample_name, pep_length, pd_epitopes_pep2, suffix):
    ...
    for row in range(pd_epitopes_pep2.shape[0]):
        ...
        if tsv_line['variant type']=='missense' or tsv_line['variant type']=='inframe_ins' or tsv_line['variant type']=='inframe_del':
            ...
            if tsv_line_fs['frameshift_amino_acid_sequence'].count(tsv_line_fs['mt epitope seq']) == 1:
                ...
                pd_epitopes_pep2.loc[row,'mt_peptide_sequence'] = tsv_line_fs['frameshift_amino_acid_sequence'][left_pos:fs_epitope_center_ind+pep_length+1]
    ...
    pd_epitopes_pep2.to_csv('pvacseq_'+ sample_name + '_flank' + str(pep_length) + 'aa_peptide_' + str(suffix) + '.txt', header=True, index=False, sep='\t')
    pd_epitopes_pep2.to_excel('pvacseq_'+ sample_name + '_flank' + str(pep_length) + 'aa_peptide_' + str(suffix) + '.xlsx', header=True, index=False, sheet_name=sample_name)
