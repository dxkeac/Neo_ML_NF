...
def filter_sites_in_hash(region_list, bam_file, ref_fasta, prefixed_sample, output_dir, insertion_centric, map_qual, base_qual, count):
    bam_readcount_cmd = ['bam-readcount', '-f', ref_fasta, '-l', region_list, '-w', '0', '-b', str(base_qual), '-q', str(map_qual), '-d', str(count)]
...
elif len(sys.argv) == 10: #elif instead of else for explicit safety
    (script_name, vcf_filename, sample, ref_fasta, bam_file, prefix, output_dir, min_base_qual, min_mapping_qual, max_count)= sys.argv
...
if len(rc_for_snp.keys()) > 0:
    ...
    filter_sites_in_hash(region_file, bam_file, ref_fasta, prefixed_sample, output_dir, False, min_mapping_qual, min_base_qual, max_count)
else:
    output_file = os.path.join(output_dir, prefixed_sample + '_bam_readcount_snv.tsv')
...
if len(rc_for_indel.keys()) > 0:
    ...
    filter_sites_in_hash(region_file, bam_file, ref_fasta, prefixed_sample, output_dir, True, min_mapping_qual, min_base_qual, max_count)
else:
    output_file = os.path.join(output_dir, prefixed_sample + '_bam_readcount_indel.tsv')
    open(output_file, 'w').close()
