...
def get_specific_length_peptide(sample_name, pep_length, pd_epitopes_pep2, suffix):
    ...
    for row in range(pd_epitopes_pep2.shape[0]):
        ...
        if tsv_line['variant type']=='inframe_fusion' or tsv_line['variant type']=='frameshift_fusion':
    ...
    pd_epitopes_pep2.to_csv('pvacfuse_'+ sample_name + '_' + str(pep_length) + 'aa_peptide_' + str(suffix) + '.txt', header=True, index=False, sep='\t')
    pd_epitopes_pep2.to_excel('pvacfuse_'+ sample_name + '_' + str(pep_length) + 'aa_peptide_' + str(suffix) + '.xlsx', header=True, index=False, sheet_name=sample_name)
